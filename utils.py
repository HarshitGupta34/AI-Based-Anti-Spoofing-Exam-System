...
EDFlag = False
#Voice Related
TRIGGER_RMS = 15  # start recording above 5
RATE = 16000  # sample rate
TIMEOUT_SECS = 0.3  # silence time after which recording stops
...
def move_file_to_output_folder(filename, folder='OutputVideos'):
...
def reduceBitRate(input_file, output_file):
    ...
    print("Video copied without bitrate reduction")
...
def faceDetectionRecording(img, text):
    ...
    if text != 'Verified Student appeared' and prev_state[0] == 'Verified Student appeared':
        ...
        for _ in range(2):
            writer[0].write(img)
    elif text != 'Verified Student appeared' and str(text) == prev_state[0] and (time.time() - start_time[0]) > 1:
        flag[0] = True
        for _ in range(2):
            writer[0].write(img)
    elif text != 'Verified Student appeared' and str(text) == prev_state[0] and (time.time() - start_time[0]) <= 1:
        ...
        if prev_state[0] != "Verified Student appeared":
            ...
            duration = math.ceil(end_time[0] - start_time[0]) 
...
def MTOP_record_duration(text, img):
    ...
    if text != 'Only one person is detected' and prev_state[2] == 'Only one person is detected':
        ...
        if not flag[2]:
            print("🚨 MTOP VIOLATION CONFIRMED")
        flag[2] = True
        for _ in range(2):
            writer[2].write(img)
    elif text != 'Only one person is detected' and str(text) == prev_state[2] and (time.time() - start_time[2]) <= 3:
        ...
        if prev_state[2] != "Only one person is detected":
            ...
            if flag[2]:
                ...
                print("🔥 WRITING MTOP JSON")
                print(MTOPViolation)
                write_json(MTOPViolation)
                print("✅ MTOP JSON SAVED")
                print(outputVideo)
                reduceBitRate(video[2], outputVideo)
                move_file_to_output_folder(outputVideo)
                print("Moved:", outputVideo)
            os.remove(video[2])
            print(recorded_durations)
...
def screenDetection():
    ...
    if new_active_window is not None:
        ...
        current_title = new_active_window.title.lower()

        print("ACTIVE WINDOW:", current_title)

        # ONLY ALLOW EXAM TAB
        if "google chrome" in current_title and "exam" in current_title:
...
def electronicDevicesDetection(frame):
    ...
    for result in detect_params:
        ...
        for box in boxes:
            ...
            # DEVICE DETECTION
            if detected_obj in ['cell phone', 'laptop', 'remote']:

                print("DEVICE DETECTED:", detected_obj)
    ...
    MTOP_record_duration(textMTOP, frame)
...
class Recorder:
        ...
        while Globalflag:
            ...
            if self.inSound(data):
                ...
                if len(sound) > 0:
                    ...
                    duration=max(1, math.floor((datetime.datetime.now()-begin_time).total_seconds()))
    ...
    def write(self, sound, begin_time, duration):
        ...
        move_file_to_output_folder(f'{filename}.wav', 'OutputVideos')
...
def cheat_Detection2():
    ...
    try:
        ...
        print("🔥 cheat_Detection2 STARTED")

        deleteTrashVideos()

        while Globalflag:

            print("🔥 CD2 LOOP RUNNING")
            ...
            electronicDevicesDetection(image2)
        ...
        print("❌ ERROR IN cheat_Detection2:", e)
#Query Related
